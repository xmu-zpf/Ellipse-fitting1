...
{                                                     \
    ...
    void RotRec2param(cv::RotatedRect& box, float* params)
    {
        ...
        C = 2 * (a * a - b * b) * sin(rot_ang) * cos(rot_ang);
        B = (a * cos(rot_ang)) * (a * cos(rot_ang)) + (b * sin(rot_ang)) * (b * sin(rot_ang));
        D = -(2 * A * cx + B * cy);
        E = -(2 * C * cy + B * cx);
        params[0] = A, params[1] = B, params[2] = C, params[3] = D, params[4] = E;
    }

    template<class T>
    static auto weight(cv::InputArray _d, int coef) ->cv::Mat
    {
        Mat ds, d = _d.getMat();
        Mat w(d.rows, 1, CV_64F);
        T median{ 0 }, tao{ 0 };
        cv::sort(d, ds, cv::SORT_ASCENDING);

#define d(x) d.at<T>(cv::Point(x,1))

        if (d.rows % 2)
        {
            median = ds.at<T>(cv::Point(d.rows / 2)) + ds.at<T>(cv::Point(d.rows / 2 - 1));
            median /= (2. * 0.6745);
        }
        else
        {
            median = ds.at<T>(cv::Point(d.rows / 2));
            median /= 0.6745;
        }
        tao = coef * median;
        ...
        {
            if (std::fabs(d(i)) > tao)
                w.at<T>(cv::Point(i, 1)) = 1e-8;
            else
            {
                double wi = (1 - (d(i) * d(i) / tao / tao));
                wi *= wi;
                if (isinf(wi))
                    w.at<T>(cv::Point(i, 1)) = 1e-8;
                else
                    w.at<T>(cv::Point(i, 1)) = wi;
            }
...
#pragma region first fit
        for (i = 0; i < n; ++i)
        {
            ...
            M.ptr<double>(i)[3] = (double)p.x;
            M.ptr<double>(i)[4] = (double)p.y;
        }
        ...
        M = Mat(2, 2, CV_64F);
        Mat N = Mat(2, 2, CV_64F);
        x = Mat(2, 1, CV_64F);
        M.ptr<double>(0)[0] = -2 * A;
        M.ptr<double>(0)[1] = M.ptr<double>(1)[0] = -B;
        M.ptr<double>(1)[1] = -2 * C;
        N.ptr<double>(0)[0] = D;
        N.ptr<double>(1)[0] = E;
        cv::solve(M, N, x, cv::DecompTypes::DECOMP_SVD);
...
#pragma region Iterations
        for (int iters = 1; iters < iterations; ++iters)
        {
            ...
            {
                auto p = ptsfc[i] - c;
                M.ptr<double>(i)[0] = (double)p.x * p.x * w.ptr<double>(i)[0];
                M.ptr<double>(i)[1] = (double)p.x * p.y * w.ptr<double>(i)[0];
                M.ptr<double>(i)[2] = (double)p.y * p.y * w.ptr<double>(i)[0];
                M.ptr<double>(i)[3] = (double)p.x * w.ptr<double>(i)[0];
                M.ptr<double>(i)[4] = (double)p.y * w.ptr<double>(i)[0];
                f.ptr<double>(i)[0] = w.ptr<double>(i)[0];
            }
            cv::solve(M, f, x, cv::DecompTypes::DECOMP_SVD);
            A = x.ptr<double>(0)[0];
            B = x.ptr<double>(1)[0];
